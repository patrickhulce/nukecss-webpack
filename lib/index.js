...
function toRegExpList(items) {
  return items.map(item => {
    return typeof item === 'string' ? new RegExp(_.escapeRegExp(item)) : item
  })
}
...
class NukeCssPlugin {
  ...
  static determineJsContent(node) {
    ...
    // Pull out just the local module identifiers from CSS files being loaded
    if (
      source.includes('/css-loader/index.js') &&
      !content.includes('removed by extract-text') &&
      !content.includes('extracted by mini-css-extract-plugin')
    ) {
